...
- `evals_slm_compaction.md` — knowledge-compaction on small local models (branch `experiment/slm-compaction`): the same long-lesson compaction methods run on 3 Ollama SLMs (llama3.1:8b, qwen2.5:7b, qwen3:8b) at a 32k window where the lesson overflows. The regime where compaction is *forced* — RAG matches/beats full-context at ~1/13th the tokens; see F24.
