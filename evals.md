...
- **F24 (2026-06-20) — On a small local model with no caching, RAG beats "shove it all"; this inverts F9/F15.** The Gemini result (full history is cheapest+best under caching) does not transfer to an SLM with a 32k window. Same long lesson (~37.7k tok), 15 questions × 7 compaction methods × 3 Ollama models (llama3.1:8b, qwen2.5:7b, qwen3:8b no-think) on an M1 Pro/16GB; judge stays on Gemini 2.5 Flash (the SLM can't hold the lesson as ground truth). `rag` is top or tied-top on all three (qwen2.5 100%, qwen3 100%, llama 80%) at ~2.9k ctx tok and ~20s/answer. `full_context` is **dominated**: equal-or-worse quality (67-80%), truncated 15/15 (37.7k→32,767), and ~12-15× slower (266-346s). `graphrag` ties `rag` on quality but costs ~2.8× tokens and ~2.5× latency (consistent with `evals_graphrag.md`). Summarization is worst, especially on the smallest model (llama `summary` 0%). Model choice matters as much as method (Qwens > llama; qwen3 most robust across methods). **Methodology fix:** the first pass under-counted quality — ~64% of judge verdicts were truncated JSON scored as fails; hardened the judge + re-graded saved answers offline (`evals/rejudge_compaction.py`, 0 unparseable after). Full writeup: `evals_slm_compaction.md`. Caveats: n=15/method, 1 trial, one lesson; treat as a ranking. (Branch `experiment/slm-compaction`, PR #3.)
- **F25 (2026-06-20) — On an SLM there is no universal best history-compaction method, and "keep everything" never wins (Axis A companion to F24, also inverts F9/F15).** Same lesson loaded turn 0 of a session, 15 question-turns, retrieval off, run through the real app middlewares (memory presets) on the 3 Ollama SLMs at a 32k window (`num_ctx` variant). The best preset is **model-dependent**: `prompt_compression` on qwen2.5 (60%), `hierarchical_summarization` on llama3.1 (40%), `summarization_only`/`delta_summarization` on qwen3 (73%). `full_history` tops no model (27%/7%/67%) and is near-bottom on the two weaker ones; `sliding_window` is reliably among the worst (13%/13%/40%). **Model capability dominates the method**: qwen3 40-73% across *every* preset >> qwen2.5 13-60% >> llama3.1 0-40%. **Mechanism finding:** on an SLM "keep everything" can't even be truncated-and-kept — Ollama's server-side context fitting *evicts the whole oversized turn-0 lesson* from turn 1 on (checkpoint holds it; the model receives ~0.3-1.4k tok), so a method that summarizes the lesson into a small injected message is the only way to retain its gist (on Gemini's window this never happens). `hierarchical_summarization` is far the most expensive (≈30-call map-reduce, p50 233-333s, ~25-29k retained tok) for no quality lead except on the weakest model. Caveats: n=15, 1 trial, LLM-judge variance moves cells ±1-3/15 (read only big gaps); full_history numbers are Ollama-eviction-specific. Writeup: `evals_slm_compaction.md` (Experiment 2). (Branch `experiment/slm-compaction`, PR #3.)
...
- `evals_slm_compaction.md` — knowledge-compaction on small local models (branch `experiment/slm-compaction`): two experiments on 3 Ollama SLMs (llama3.1:8b, qwen2.5:7b, qwen3:8b) at a 32k window where the lesson overflows. **Axis B** (fit a document: RAG vs stuff vs summarize, F24) and **Axis A** (compact growing history via the real memory presets, F25). The regime where compaction is *forced*; RAG wins Axis B, no universal winner on Axis A, and "keep everything" never wins on an SLM.
